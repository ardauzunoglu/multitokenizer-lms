...
# Resume the single_large baseline from the checkpoint selected by latest.txt.
# Optional overrides:
#   NPROC_PER_NODE=2  Number of local ranks (must match the config's dp=2)
#   MASTER_PORT=29502 Rendezvous port; use a distinct port per concurrent run
#   LOG_FILE=...      Existing log to append combined stdout/stderr to

script_dir="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
repo_dir="$(cd -- "$script_dir/.." && pwd)"
config_file="$script_dir/config_dclm_stack_20m_single_large_10b_seq4096_seed1000_resume.yaml"
...
if [[ ! -f "$config_file" ]]; then
  echo "Missing resume config: $config_file" >&2
  exit 2
fi

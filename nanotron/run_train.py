...
from nanotron.config import (
    DataArgs,
    DatasetStageArgs,
    MultiTokenizerNanosetDatasetsArgs,
    NanosetDatasetsArgs,
...
from nanotron.data.multi_tokenizer import HomogeneousDataLoader, HomogeneousTokenizerStepSampler, MultiTokenizerDatasetManifest, validate_source_manifest
...
def get_dataloader_from_data_stage(
    ...
    elif isinstance(data.dataset, MultiTokenizerNanosetDatasetsArgs):
        from nanotron.data.tokenized_bytes import get_tb_dataloader, get_tb_datasets
        if trainer.vocabulary_config is None:
            raise RuntimeError("Multi-tokenizer dataset configured without a resolved vocabulary registry")
        sources_by_tokenizer, manifests_by_tokenizer = defaultdict(list), defaultdict(list)
        for source in data.dataset.sources:
            manifest = MultiTokenizerDatasetManifest.load(f"{source.path}/multi_tokenizer_manifest.json")
            validate_source_manifest(manifest, trainer.vocabulary_config.by_id(source.tokenizer_id))
            sources_by_tokenizer[source.tokenizer_id].append(source)
            manifests_by_tokenizer[source.tokenizer_id].append(manifest)
        expected_ids = set(range(len(trainer.vocabulary_config.vocabularies)))
        if set(sources_by_tokenizer) != expected_ids:
            raise ValueError(f"Every tokenizer needs a source; expected {sorted(expected_ids)}, got {sorted(sources_by_tokenizer)}")
        per_tokenizer_loaders = {}
        current_stage_metadata = trainer.metadata.current_stage
        for tokenizer_id in sorted(sources_by_tokenizer):
            sources, manifests = sources_by_tokenizer[tokenizer_id], manifests_by_tokenizer[tokenizer_id]
            if len({m.token_size_in_bytes for m in manifests}) != 1:
                raise ValueError(f"All sources for tokenizer {tokenizer_id} must use the same token storage width")
            read_paths = [source.read_path for source in sources]
            nanoset_config = NanosetDatasetsArgs(
                dataset_folder=[source.path for source in sources], dataset_weights=[source.weight for source in sources],
                dataset_read_path=read_paths if any(path is not None for path in read_paths) else None,
                tokenizer_name=manifests[0].tokenizer_name_or_path, vocab_size=manifests[0].original_vocab_size,
                token_size_in_bytes=manifests[0].token_size_in_bytes, return_positions=data.dataset.return_positions,
                skip_in_stream=data.dataset.skip_in_stream, pad_samples_to_global_batch_size=data.dataset.pad_samples_to_global_batch_size,
                shuffle_files=data.dataset.shuffle_files,
            )
            consumed = current_stage_metadata.consumed_samples_per_tokenizer.get(str(tokenizer_id), 0)
            tokenizer_dataset, _ = get_tb_datasets(
                config=nanoset_config, global_batch_size=trainer.global_batch_size, sequence_length=trainer.sequence_length,
                train_steps=trainer.config.tokens.train_steps, current_iteration=trainer.iteration_step,
                parallel_context=trainer.parallel_context, shuffle=nanoset_config.shuffle_files,
                eos_token_id=manifests[0].eos_token_id, seed=data.seed, consumed_samples=consumed,
                consumed_tokens_per_dataset_folder=consumed_tokens_per_dataset_folder,
                last_stages_consumed_tokens_per_dataset_folder=last_stages_consumed_tokens_per_dataset_folder,
            )
            per_tokenizer_loaders[tokenizer_id] = get_tb_dataloader(
                dataset=tokenizer_dataset, sequence_length=trainer.sequence_length, micro_batch_size=trainer.micro_batch_size,
                global_batch_size=trainer.global_batch_size, num_workers=data.num_loading_workers, cfg=nanoset_config,
                consumed_samples=consumed, num_samples=trainer.config.tokens.train_steps * trainer.global_batch_size,
                parallel_context=trainer.parallel_context, input_pp_rank=input_pp_rank, output_pp_rank=output_pp_rank,
                tokenizer_id=tokenizer_id,
            )
        schedule = trainer.config.multi_tokenizer.schedule
        weights = schedule.weights or [sum(s.weight for s in sources_by_tokenizer[k]) for k in sorted(sources_by_tokenizer)]
        compression = [sum(m.source_bytes for m in manifests_by_tokenizer[k]) / sum(m.tokens for m in manifests_by_tokenizer[k]) for k in sorted(manifests_by_tokenizer)]
        scheduler = HomogeneousTokenizerStepSampler(weights, strategy=schedule.strategy, seed=schedule.seed,
            weight_unit=schedule.weight_unit, bytes_per_token=compression if schedule.weight_unit == "source_bytes" else None)
        if current_stage_metadata.tokenizer_scheduler_state is not None:
            scheduler.load_state_dict(current_stage_metadata.tokenizer_scheduler_state)
        dataloader = HomogeneousDataLoader(per_tokenizer_loaders, scheduler, trainer.n_micro_batches_per_batch)

    else:
        raise ValueError(f"Unhandled case of `self.config.data.dataset`. Got: {data.dataset}")

    if sanity_check_dataloader_interval is not None and trainer.config.tokenizer is not None:

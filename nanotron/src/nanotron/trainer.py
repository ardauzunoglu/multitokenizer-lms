...
from nanotron.config.vocabulary import resolve_multi_vocabulary, validate_vocabulary_manifest
...
class DistributedTrainer:
    def __init__(
        ...
        self.vocabulary_config = (resolve_multi_vocabulary(
            self.config.multi_tokenizer,
            tp_size=self.config.parallelism.tp,
            make_vocab_size_divisible_by=self.config.model.make_vocab_size_divisible_by,
        ) if self.config.multi_tokenizer is not None else None)
        ...
        with prof:
            for self.iteration_step in range(self.initial_iter_step, self.last_iter_step + 1):
                ...
                if self.vocabulary_config is not None:
                    tokenizer_id = getattr(self.current_base_dl, "active_tokenizer_id", None)
                    if tokenizer_id is None:
                        raise RuntimeError("Multi-tokenizer dataloader did not expose its active tokenizer")
                    key = str(tokenizer_id)
                    stage_metadata = self.metadata.current_stage
                    stage_metadata.consumed_samples_per_tokenizer[key] = stage_metadata.consumed_samples_per_tokenizer.get(key, 0) + self.global_batch_size
                    stage_metadata.consumed_tokens_per_tokenizer[key] = stage_metadata.consumed_tokens_per_tokenizer.get(key, 0) + self.global_batch_size * self.sequence_length
                    stage_metadata.active_steps_per_tokenizer[key] = stage_metadata.active_steps_per_tokenizer.get(key, 0) + 1
                    stage_metadata.tokenizer_scheduler_state = self.current_base_dl.state_dict()
    ...
    def training_step(
        ...
        active_tokenizer_id = getattr(self.current_base_dl, "active_tokenizer_id", None) if self.vocabulary_config is not None else None
        if active_tokenizer_id is not None and self.parallel_context.dp_pg.size() > 1:
            gathered_ids = [None] * self.parallel_context.dp_pg.size()
            dist.all_gather_object(gathered_ids, active_tokenizer_id, group=self.parallel_context.dp_pg)
            if len(set(gathered_ids)) != 1:
                raise RuntimeError(f"Data-parallel ranks selected different tokenizer IDs: {gathered_ids}")
        # Sync tied weights
        if not isinstance(self.model, DistributedDataParallel):
            ...
            sync_gradients_across_dp(
                ...
                grad_accumulator=self.grad_accumulator,
                active_tokenizer_id=active_tokenizer_id,
            )
    ...
    def _init_model_instance(self) -> NanotronModel:
        ...
        model_kwargs = {"config": self.model_config, "parallel_context": self.parallel_context,
                        "parallel_config": self.config.parallelism, "random_states": self.random_states}
        if self.vocabulary_config is not None:
            if model_config_cls != "LlamaConfig":
                raise ValueError("Multi-tokenizer mode currently supports LlamaConfig only")
            model_kwargs["vocabulary_config"] = self.vocabulary_config
        model = self._init_model(model_builder=lambda: CONFIG_TO_MODEL_CLASS[model_config_cls](**model_kwargs))
        return model
    ...
    def _load_model_checkpoint(self, model: NanotronModel) -> NanotronModel:
        ...
        if self.init_checkpoint_path is not None and self.vocabulary_config is not None:
            manifest_path = Path(self.init_checkpoint_path) / "multi_tokenizer_manifest.json"
            if not manifest_path.exists():
                raise ValueError(f"Multi-tokenizer checkpoint is missing {manifest_path.name}")
            with manifest_path.open() as stream:
                validate_vocabulary_manifest(self.vocabulary_config, json.load(stream))
    ...
    def save_checkpoint(self) -> Path:
        ...
        if self.vocabulary_config is not None and dist.get_rank(self.parallel_context.world_pg) == 0:
            with (checkpoint_path / "multi_tokenizer_manifest.json").open("w") as stream:
                json.dump(self.vocabulary_config.to_manifest(), stream, indent=2, sort_keys=True)
                stream.write("\n")
...
def mark_tied_parameters(
    ...
    # Tie embeddings
    for embeddings_lm_head_tied_names in model.get_embeddings_lm_head_tied_groups():
        shared_embeddings = [
            (
                target,
                (

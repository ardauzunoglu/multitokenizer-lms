...
class NanotronParameter(nn.Parameter):
    ...
    def get_metadata(self, key: str, default: Any = None) -> Any:
        return getattr(self, self.NANOTRON_PARAMETER_METADATA_ATTRIBUTE_NAME).get(key, default)
...
def endpoint_id(parameter: nn.Parameter):
    return parameter.get_metadata("tokenizer_endpoint_id") if isinstance(parameter, NanotronParameter) else None


def is_inactive_endpoint(parameter: nn.Parameter, active_tokenizer_id: int) -> bool:
    parameter_tokenizer_id = endpoint_id(parameter)
    return parameter_tokenizer_id is not None and parameter_tokenizer_id != active_tokenizer_id

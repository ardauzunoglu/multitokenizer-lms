...
def sync_gradients_across_dp(
    ...
    **sync_options,
    ...
    if grad_accumulator is not None:
        ...
        grad_accumulator.sync_gradients_across_dp(dp_pg=dp_pg, reduce_op=reduce_op, active_tokenizer_id=active_tokenizer_id, **sync_options)
        return

    # Sync gradients
    for name, param in module.named_parameters():
        if active_tokenizer_id is not None and is_inactive_endpoint(param, active_tokenizer_id):
            if param.grad is not None:
                raise RuntimeError(f"Inactive tokenizer endpoint {name} unexpectedly has a gradient")
            continue
        if param.grad is None:
            raise RuntimeError(f"Active parameter {name} has no gradient")

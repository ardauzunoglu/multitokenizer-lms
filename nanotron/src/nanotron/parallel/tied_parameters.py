...
def sync_tied_weights_gradients(
    ...
    for (name, group_ranks), tied_param in sorted(tied_id_to_param.items(), key=lambda x: x[0]):
        if active_tokenizer_id is not None:
            ...
            if is_inactive_endpoint(tied_param, active_tokenizer_id):
                continue
        ...
        if grad_accumulator is not None:
            if hasattr(grad_accumulator, "has_real_grad") and not grad_accumulator.has_real_grad(name):
                raise RuntimeError(f"Active tied parameter {name} has no accumulated gradient")
            tied_grad = grad_accumulator.get_grad_buffer(name=name)
        else:
            tied_grad = tied_param.grad
            if tied_grad is None:
                raise RuntimeError(f"Active tied parameter {name} has no gradient")
        log_rank(

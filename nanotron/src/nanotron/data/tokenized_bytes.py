...
from nanotron.data.clm_collator import DataCollatorForMultiTokenizerCLM, DataCollatorForMultiTokenizerCLMWithPositionIds
from nanotron.data.multi_tokenizer import TokenizerTaggedDataset
...
def get_tb_dataloader(
    ...
    if dist.get_rank(parallel_context.pp_pg) not in [
        ...
        dataset = TokenizerTaggedDataset(dataset, tokenizer_id)
    ...
    if use_position_ids:
        collator_class = DataCollatorForMultiTokenizerCLMWithPositionIds if tokenizer_id is not None else DataCollatorForCLMWithPositionIds
        ...
        collator_class = DataCollatorForMultiTokenizerCLM if tokenizer_id is not None else DataCollatorForCLM
        data_collator = collator_class(
            sequence_length=sequence_length,
            input_pp_rank=input_pp_rank,
            output_pp_rank=output_pp_rank,

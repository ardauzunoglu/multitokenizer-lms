...
def _add_tokenizer_ids(result, examples, input_pp_rank, output_pp_rank, parallel_context):
    current_pp_rank = dist.get_rank(parallel_context.pp_pg)
    result["input_tokenizer_id"] = TensorPointer(group_rank=input_pp_rank)
    result["label_tokenizer_id"] = TensorPointer(group_rank=output_pp_rank)
    if current_pp_rank not in (input_pp_rank, output_pp_rank):
        return result
    if not examples or any("tokenizer_id" not in example for example in examples):
        raise ValueError("Every multi-tokenizer example must contain tokenizer_id")
    tokenizer_ids = [int(np.asarray(example["tokenizer_id"]).item()) for example in examples]
    if len(set(tokenizer_ids)) != 1:
        raise ValueError(f"A microbatch must contain exactly one tokenizer ID; got {sorted(set(tokenizer_ids))}")
    tokenizer_id = torch.tensor(tokenizer_ids[0], dtype=torch.long)
    if current_pp_rank == input_pp_rank:
        result["input_tokenizer_id"] = tokenizer_id
    if current_pp_rank == output_pp_rank:
        result["label_tokenizer_id"] = tokenizer_id
    return result


@dataclasses.dataclass
class DataCollatorForMultiTokenizerCLM(DataCollatorForCLM):
    def __call__(self, examples):
        return _add_tokenizer_ids(super().__call__(examples), examples, self.input_pp_rank, self.output_pp_rank, self.parallel_context)
...
@dataclasses.dataclass
class DataCollatorForMultiTokenizerCLMWithPositionIds(DataCollatorForCLMWithPositionIds):
    def __call__(self, examples):
        return _add_tokenizer_ids(super().__call__(examples), examples, self.input_pp_rank, self.output_pp_rank, self.parallel_context)

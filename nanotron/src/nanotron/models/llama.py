...
from nanotron.models.multi_tokenizer import DisjointEmbeddingBank, DisjointLMHeadBank, checked_scalar_id
...
class MultiTokenizerEmbedding(nn.Module, AttachableStore):
    def __init__(self, tp_pg, config, parallel_config, vocabulary_config):
        super().__init__()
        self.token_embeddings = DisjointEmbeddingBank(
            vocabulary_config, config.hidden_size, config.pad_token_id, tp_pg,
            parallel_config.tp_mode if parallel_config is not None else TensorParallelLinearMode.ALL_REDUCE,
        )

    def forward(self, input_ids, input_mask, input_tokenizer_id):
        store = self.get_local_store()
        if store is not None:
            past_length = store.get("past_length", torch.zeros(1, dtype=torch.long, device=input_ids.device).expand(input_ids.shape[0]))
            store["past_length"] = past_length + input_mask.cumsum(-1, dtype=torch.long)[:, -1]
        return {"input_embeds": self.token_embeddings(input_ids.transpose(0, 1), input_tokenizer_id)}
...
class LlamaModel(nn.Module):
    ...
    def __init__(
        ...
        embedding_builder = Embedding if vocabulary_config is None else MultiTokenizerEmbedding
        embedding_kwargs = {"tp_pg": parallel_context.tp_pg, "config": config, "parallel_config": parallel_config}
        embedding_input_keys = {"input_ids", "input_mask"}
        if vocabulary_config is not None:
            embedding_kwargs["vocabulary_config"] = vocabulary_config
            embedding_input_keys.add("input_tokenizer_id")
        self.token_position_embeddings = PipelineBlock(
            p2p=self.p2p,
            module_builder=embedding_builder,
            module_kwargs=embedding_kwargs,
            module_input_keys=embedding_input_keys,
        ...
        legacy_lm_head_kwargs = {
            "in_features": config.hidden_size, "out_features": config.vocab_size, "pg": parallel_context.tp_pg,
            "bias": False, "mode": self.tp_mode, "async_communication": tp_linear_async_communication,
            "tp_recompute_allgather": parallel_config.tp_recompute_allgather,
        }
        multi_lm_head_kwargs = {
            "vocabulary_config": vocabulary_config, "hidden_size": config.hidden_size, "tp_pg": parallel_context.tp_pg,
            "mode": self.tp_mode, "async_communication": tp_linear_async_communication,
            "tp_recompute_allgather": parallel_config.tp_recompute_allgather,
        }
        self.lm_head = PipelineBlock(
            p2p=self.p2p,
            # Understand that this means that we return sharded logits that are going to need to be gathered
            module_builder=TensorParallelColumnLinear if vocabulary_config is None else DisjointLMHeadBank,
            module_kwargs=legacy_lm_head_kwargs if vocabulary_config is None else multi_lm_head_kwargs,
            module_input_keys={"x"} if vocabulary_config is None else {"hidden_states", "tokenizer_id"},
    ...
    def forward(
        ...
        input_tokenizer_id=None,
        label_tokenizer_id=None,
    ):
        return self.forward_with_hidden_states(input_ids, input_mask, input_tokenizer_id, label_tokenizer_id)[0]
    ...
    def forward_with_hidden_states(
        ...
        if self.vocabulary_config is None:
            output = self.token_position_embeddings(input_ids=input_ids, input_mask=input_mask)
        else:
            if input_tokenizer_id is None or label_tokenizer_id is None:
                raise ValueError("Multi-tokenizer Llama requires input_tokenizer_id and label_tokenizer_id")
            if not isinstance(input_tokenizer_id, TensorPointer) and not isinstance(label_tokenizer_id, TensorPointer):
                if checked_scalar_id(input_tokenizer_id, len(self.vocabulary_config.vocabularies)) != checked_scalar_id(label_tokenizer_id, len(self.vocabulary_config.vocabularies)):
                    raise ValueError("Input and label tokenizer IDs must match")
            output = self.token_position_embeddings(input_ids=input_ids, input_mask=input_mask, input_tokenizer_id=input_tokenizer_id)
        ...
        sharded_logits = (self.lm_head(x=hidden_states) if self.vocabulary_config is None else self.lm_head(hidden_states=hidden_states, tokenizer_id=label_tokenizer_id))["logits"]
    ...
    def get_block_compute_costs(self):
        ...
        if self.vocabulary_config is not None:
            block_compute_costs[DisjointLMHeadBank] = max(v.padded_vocab_size for v in self.vocabulary_config.vocabularies) * model_config.hidden_size
...
class LlamaForTraining(NanotronModel):
    def __init__(
        ...
        super().__init__()
        self.vocabulary_config = vocabulary_config
        self.model = LlamaModel(config=config, parallel_context=parallel_context, parallel_config=parallel_config, vocabulary_config=vocabulary_config)
    ...
    def forward(
        ...
        sharded_logits = self.model(
            ...
            input_tokenizer_id=input_tokenizer_id,
            label_tokenizer_id=label_tokenizer_id,
        )
        if self.vocabulary_config is not None and not isinstance(label_ids, TensorPointer):
            vocabulary = self.vocabulary_config.by_id(checked_scalar_id(label_tokenizer_id, len(self.vocabulary_config.vocabularies)))
            if label_ids.numel() and (label_ids.min() < 0 or label_ids.max() >= vocabulary.original_vocab_size):
                raise ValueError(f"Label token IDs for {vocabulary.name!r} must be in [0, {vocabulary.original_vocab_size})")
        loss = self.loss(
    ...
    def get_embeddings_lm_head_tied_groups(self):
        if self.vocabulary_config is None:
            return super().get_embeddings_lm_head_tied_groups()
        if not self.vocabulary_config.tie_word_embeddings:
            return []
        return [[
            f"model.token_position_embeddings.pp_block.token_embeddings.embeddings.{v.id}.weight",
            f"model.lm_head.pp_block.heads.{v.id}.weight",
        ] for v in self.vocabulary_config.vocabularies]

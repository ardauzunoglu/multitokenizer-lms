...
@dataclass
class MultiTokenizerSourceArgs:
    path: str
    tokenizer_id: int
    weight: float = 1.0
    ...
    def __post_init__(self):
        if self.tokenizer_id < 0:
            raise ValueError("tokenizer_id must be non-negative")
        if self.weight <= 0:
            raise ValueError("multi-tokenizer source weights must be positive")


@dataclass
class MultiTokenizerNanosetDatasetsArgs:
    sources: List[MultiTokenizerSourceArgs]
    type: Literal["multi_tokenizer_nanoset"] = "multi_tokenizer_nanoset"
    return_positions: bool = True
    skip_in_stream: bool = False
    pad_samples_to_global_batch_size: bool = False
    shuffle_files: bool = False

    def __post_init__(self):
        if self.type != "multi_tokenizer_nanoset":
            raise ValueError(f"Unsupported multi-tokenizer dataset type: {self.type}")
        if not self.sources:
            raise ValueError("multi-tokenizer datasets require at least one source")
...
class DataArgs:
    ...
    dataset: Optional[
        Union[PretrainDatasetsArgs, NanosetDatasetsArgs, MultiTokenizerNanosetDatasetsArgs, SFTDatasetsArgs]
...
class TokenizerScheduleArgs:
    ...
    def __post_init__(self):
        if self.strategy not in ("weighted_deficit", "round_robin"):
            raise ValueError(f"Unsupported tokenizer schedule strategy: {self.strategy}")
        if self.weight_unit not in ("tokens", "source_bytes"):
            raise ValueError(f"Unsupported tokenizer schedule weight unit: {self.weight_unit}")
        if self.weights is not None and (not self.weights or any(weight <= 0 for weight in self.weights)):
            raise ValueError("Tokenizer schedule weights must be positive")


@dataclass
class MultiTokenizerArgs:
    tokenizers: List[TokenizerSpec]
    schedule: TokenizerScheduleArgs = field(default_factory=TokenizerScheduleArgs)
    batching: Literal["step_homogeneous"] = "step_homogeneous"
    tie_word_embeddings: bool = True
    mask_padded_vocab_logits: bool = True

    def __post_init__(self):
        if not isinstance(self.schedule, TokenizerScheduleArgs):
            raise TypeError("multi_tokenizer.schedule must be a TokenizerScheduleArgs")
        self.schedule.__post_init__()
        if self.batching != "step_homogeneous":
            raise ValueError("Only step_homogeneous multi-tokenizer batching is supported")
        if len(self.tokenizers) < 2:
            raise ValueError("Multi-tokenizer mode requires at least two tokenizers")
        ids = [tokenizer.id for tokenizer in self.tokenizers]
        if ids != list(range(len(ids))):
            raise ValueError(f"Tokenizer IDs must be unique, contiguous, and ordered from zero; got {ids}")
        names = [tokenizer.name for tokenizer in self.tokenizers]
        if len(set(names)) != len(names):
            raise ValueError(f"Tokenizer names must be unique; got {names}")
        if any(not tokenizer.name or not tokenizer.tokenizer_name_or_path for tokenizer in self.tokenizers):
            raise ValueError("Every tokenizer requires a nonempty name and tokenizer_name_or_path")
        if self.schedule.weights is not None and len(self.schedule.weights) != len(self.tokenizers):
            raise ValueError("Tokenizer schedule weights must match the tokenizer registry length")
...
class Config:
    ...
    def __post_init__(self):
        ...
        if self.tokenizer is not None and self.multi_tokenizer is not None:
            raise ValueError("Configure exactly one of tokenizer and multi_tokenizer, not both")
        if self.multi_tokenizer is not None:
            self.multi_tokenizer.__post_init__()
            if self.lighteval is not None:
                raise ValueError("LightEval is not yet supported for multi-tokenizer configs; evaluate an extracted endpoint")
        ...
        if self.data_stages is not None:
            ...
            for stage in self.data_stages:
                ...
                if isinstance(stage.data.dataset, MultiTokenizerNanosetDatasetsArgs):
                    if self.multi_tokenizer is None:
                        raise ValueError("multi_tokenizer_nanoset requires a top-level multi_tokenizer registry")
                    known_ids = {tokenizer.id for tokenizer in self.multi_tokenizer.tokenizers}
                    present_ids = {source.tokenizer_id for source in stage.data.dataset.sources}
                    unknown_ids = present_ids - known_ids
                    if unknown_ids:
                        raise ValueError(f"Dataset stage {stage.name!r} references unknown tokenizer IDs: {sorted(unknown_ids)}")
                    if present_ids != known_ids:
                        raise ValueError(f"Dataset stage {stage.name!r} must contain every tokenizer ID; expected {sorted(known_ids)}, got {sorted(present_ids)}")
                    registry = {tokenizer.id: tokenizer for tokenizer in self.multi_tokenizer.tokenizers}
                    for source in stage.data.dataset.sources:
                        manifest_path = Path(source.path) / "multi_tokenizer_manifest.json"
                        if not manifest_path.exists():
                            raise ValueError(f"Multi-tokenizer source is missing manifest: {manifest_path}")
                        with manifest_path.open() as stream:
                            manifest = json.load(stream)
                        spec = registry[source.tokenizer_id]
                        if manifest.get("tokenizer_id") != source.tokenizer_id:
                            raise ValueError(f"Manifest {manifest_path} has the wrong tokenizer_id")
                        if spec.fingerprint is not None and manifest.get("tokenizer_fingerprint") != spec.fingerprint:
                            raise ValueError(f"Manifest {manifest_path} fingerprint does not match tokenizer {spec.name!r}")
                        if spec.vocab_size is not None and manifest.get("original_vocab_size") != spec.vocab_size:
                            raise ValueError(f"Manifest {manifest_path} vocabulary size does not match tokenizer {spec.name!r}")

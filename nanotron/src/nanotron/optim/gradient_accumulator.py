...
class GradientAccumulator(ABC):
    ...
    @abstractmethod
    def sync_gradients_across_dp(self, dp_pg: dist.ProcessGroup, reduce_op: dist.ReduceOp, reduce_scatter: bool, active_tokenizer_id=None):
        ...
...
class FP32GradientAccumulator(GradientAccumulator):
    def __init__(
        ...
        self.has_grad = {name: False for name in self.fp32_grad_buffers}
    ...
    def sync_gradients_across_dp(self, dp_pg: dist.ProcessGroup, reduce_op: dist.ReduceOp, reduce_scatter: bool, active_tokenizer_id=None):
        ...
        if reduce_scatter:
            ...
            if active_tokenizer_id is not None:
                from nanotron.parallel.parameters import is_inactive_endpoint
                named_offsets = [item for item in named_offsets if not is_inactive_endpoint(self.fp32_grad_buffers[item[0]]["half"], active_tokenizer_id)]
            ...
            if active_tokenizer_id is None:
                dist.all_reduce(self._contiguous_fp32_grad_buffer, op=reduce_op, group=dp_pg)
            else:
                from nanotron.parallel.parameters import is_inactive_endpoint
                for name, entry in self.fp32_grad_buffers.items():
                    if is_inactive_endpoint(entry["half"], active_tokenizer_id):
                        continue
                    if not self.has_grad[name]:
                        raise RuntimeError(f"Active parameter {name} has no accumulated gradient")
                    dist.all_reduce(entry["fp32_grad"], op=reduce_op, group=dp_pg)
    ...
    def zero_grad(self):
        ...
        for name in self.has_grad:
            self.has_grad[name] = False
            if name in self.parameters:
                self.parameters[name]["fp32"].grad = None

    def has_real_grad(self, name: str) -> bool:
        return self.has_grad[name]

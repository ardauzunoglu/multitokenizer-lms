...
\title{Multi-Tokenizer Language Models: Shorter Sequences with Small Per-Example Output Heads}
\date{}
...
\centering
\scriptsize
\caption{Tokenizer compression results.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{llrrrrr}
\toprule
Evaluation text & Tokenization & Active / stored vocab. & Tokens & Bytes/token & Tokens/KiB & Token reduction \\
\midrule
...
\centering
\scriptsize
\caption{Router results.}
\begin{tabular}{lrrrrl}
\toprule
Evaluation set & Documents & Agreement & Regret & Training acc. & Learned / oracle usage (0/1/2) \\
\midrule
DCLM + Stack & 11,286 & 97.17\% & 0.264\% & 96.38\% & 9,147 / 14 / 2,125; 9,002 / 156 / 2,128 \\
...
The 0.264\% mixture token regret means routing accounts for only 0.26 percentage points of the mixture's 3.75\% oracle compression gain.

\section{Language-model cross-entropy evaluation}
...
\centering
\scriptsize
\caption{Held-out language-model cross-entropy.}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrrr}
\toprule
Model / routed endpoint & Update & Documents & Predicted tokens & CE (nats/token) & Perplexity & CE (nats/byte) & Bits/byte \\
\midrule
...
\section{Training loss across training}

Each run used the same nominal 10B-token schedule (9,537 updates; 1.05M tokens/update). The single-49K run stopped at update 9,268 (9.72B tokens). Figure~\ref{fig:training-loss} shows the logged training LM loss rather than tabulating it. These per-token losses are useful optimization diagnostics but are not directly comparable across tokenizer strategies.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{eval/train_lm_loss.png}
  \caption{Training LM loss by update for the multi-tokenizer, single-16K, and single-49K strategies.}
  \label{fig:training-loss}
\end{figure}

\section{Next step}

The immediate experiment is to rerun at matched \emph{byte} and wall-clock budgets, profile output projection separately from the Transformer body, and improve the underperforming endpoint's data balance and routing. Success requires better bits/byte than the single-16K baseline together with a measured---not presumed---end-to-end efficiency gain.
